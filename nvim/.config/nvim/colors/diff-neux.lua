...
local sea_foam  = hsluv(208, 80, 80)  -- Vim has a mapping, <n>C-a and <n>C-x to
local sea_crest = hsluv(208, 90, 30)  -- increment or decrement integers, or
local sea_deep  = hsluv(208, 90, 10)  -- you can just type them normally.
local sea_gull  = hsluv("#c6c6c6")    -- Or use hex form, preceeded with a #.

local black = hsluv(50, 15, 4)
local white = hsluv(50, 20, 96)

local blend_ratio = 15
local diff_green = hsluv(117, 100, 40).mix(black, blend_ratio)
local diff_red = hsluv(12, 100, 40).mix(black, blend_ratio)
local diff_yellow = hsluv(49, 100, 40).mix(black, blend_ratio)
...
local theme = lush(function()
  return {
    ...
    Whitespace { fg = Normal.bg.saturate(10).lighten(51) },
    ...
    -- search_base  { bg = hsluv(52, 52, 52), fg = hsluv(52, 10, 10) },
    -- Search       { search_base },
